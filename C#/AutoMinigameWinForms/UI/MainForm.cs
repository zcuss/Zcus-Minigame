...
public sealed class MainForm : Form
{
    ...
    private double _ocrDbgRotationAccumDeg;
    private double? _ocrDbgLastAngleDeg;
    ...
    private void RestartRoundWithoutTimeout(string key, double nowMs)
    {
        if (string.IsNullOrWhiteSpace(key))
        {
            return;
    ...
    private void Loop()
    {
        ...
        if (result.RedAngle.HasValue && _prevRedAngleDeg.HasValue && _prevRedAngleMs > 0)
        {
            var dtMs = nowMs - _prevRedAngleMs;
            if (dtMs > 0.9)
            {
                var delta = SignedDeltaDeg(_prevRedAngleDeg.Value, result.RedAngle.Value);
                redSpeedDegPerSec = delta * (1000.0 / dtMs);
            }
        }
        ...
        if (_debugAllLogs && result.RedAngle.HasValue)
        {
            ...
            {
                _ocrDbgRotationAccumDeg += Math.Abs(SignedDeltaDeg(_ocrDbgLastAngleDeg.Value, redNow));
            }

            _ocrDbgLastAngleDeg = redNow;
        }
        ...
        if (!string.IsNullOrWhiteSpace(reliableKey))
        {
            ...
            {
                // Prevent rapid re-key churn when OCR flickers between letters mid-round.
                var roundAgeMs = Math.Max(0.0, nowMs - _roundStartMs);
                var allowRoundSwitch = ocrRoundAccepted && roundAgeMs <= AppConstants.WasdRoundSwitchWindowMs;
                if (allowRoundSwitch)
                {
                    RestartRoundWithoutTimeout(reliableKey, nowMs);
                }
            }
        }
        ...
        var timeToCenterMs = (double?)null;
        if (signedToCenterDeg.HasValue && redSpeedDegPerSec.HasValue && Math.Abs(redSpeedDegPerSec.Value) >= AppConstants.PressPredictionMinSpeedDegPerSec)
        {
            var tMs = (signedToCenterDeg.Value / redSpeedDegPerSec.Value) * 1000.0;
            if (tMs >= 0.0 && tMs <= 900.0)
            {
                timeToCenterMs = tMs;
            }
        }

        if (_roundState == RoundState.Armed && timeToCenterMs.HasValue)
        {
        ...
        if (_roundState == RoundState.Candidate &&
            ...
            (nowMs - _roundScheduleFireAtMs) > 180.0)
        {
            _roundScheduleFireAtMs = double.PositiveInfinity;
        }
        ...
        var wasdDeadlineRescueFireReady = false;
        ...
        {
            var deepRoundAgeMs = nowMs - _roundStartMs;
            var deepDeadline = deepRoundAgeMs >= (AppConstants.RoundTimeoutMs * 0.97);
            var hasTimingSignal =
        ...
        var fallbackFireNow = canPress && (fallbackDue || hardFallbackDue || deadlineRescueFireReady || wasdDeadlineRescueFireReady);
        ...
        if (_debugAllLogs)
        {
            ...
            var rejectReason = string.Join(',',
                new[]
                {
                    ...
                    directTouchFireReady || schedulerFireReady || directEdgeFireReady || fallbackFireReady || opportunisticFireReady || hardFallbackFireReady || deadlineRescueFireReady || wasdDeadlineRescueFireReady ? null : "wait-schedule",
            ...
            var dbgAction = canPress || directTouchFireReady || schedulerFireReady || directEdgeFireReady || fallbackFireReady || opportunisticFireReady || hardFallbackFireReady || deadlineRescueFireReady || wasdDeadlineRescueFireReady;
